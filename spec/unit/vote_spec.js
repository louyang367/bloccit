...
const getPost = require("../../src/db/queries.posts").getPost;
...
describe("Vote", () => {
  ...
  describe("#create()", () => {
    ...
    it("should not create an upvote of greater than +1 on a post for a user", (done) => {
      ...
      Vote.create({
        ...
        .catch((err) => {
          console.log(err);
          expect(JSON.stringify(err)).toContain('Validation isIn on value failed');
          done();
        });
    });

    it("should not create a downvote of less than -1 on a post for a user", (done) => {
  ...
  describe("#getPoints()", () => {
    ...
    it("should return the cumulative number of votes on a post", (done) => {
      Vote.create({
        value: -1,
        postId: this.post.id,
        userId: this.user.id
  ...
  describe("#hasUpvoteFor() & #hasDownvoteFor()", () => {

    it("should return true if the user with the matching userId has an upvote for the post for hasUpvoteFor(), and the opposite for hasDownvoteFor()", (done) => {
      Vote.create({
        value: -1,
        postId: this.post.id,
        userId: this.user.id
        ...
        .then((vote) => {
          ...
          getPost(this.post.id, (err, post) => {
            if (err) console.log(err)
            //first vote is -1 by this.user.id
              ...
              .then((user2) => {
                ...
                Vote.create({
                  ...
                  .then((vote) => {
                    ...
                    getPost(this.post.id, (err, post) => {
                      if (err) console.log(err)
                      //second vote from user2 is +1
                      expect(post.hasUpvoteFor(this.user2.id)).toBe(true);
                      expect(post.hasDownvoteFor(this.user2.id)).toBe(false);
                      done();
                    })
                  })
              })
